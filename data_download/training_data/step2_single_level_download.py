...
def main(args):
    ...
    output_dir = f"{config.DATA_DIR}/single_level_1.5" 
    ...
    for single_date in date_range:
        ...
        daily_data = a.sel(time=date_str)[config.ERA5_SINGLE_LEVEL]
        yy, mm, dd = daily_data.time.dt.strftime('%Y-%m-%d').item().split('-')

        daily_data=daily_data.isel(latitude=lat_idx, longitude=lon_idx)

        output_daily_file = f"{output_dir}/era5_single_full_1.5deg_{yy}{mm}{dd}.zarr"
