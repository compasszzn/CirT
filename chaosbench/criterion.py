...
class RMSE_GRID(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        mean_squared_error = torch.nanmean(squared_diff,dim=0)
...
class RMSE_TIME(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # Calculate the mean squared error
        mean_squared_error = torch.nanmean(squared_diff,dim=(1,2))
        # Take the square root to get the RMSE
        rmse = torch.sqrt(mean_squared_error)
        
        return rmse

class RMSE_LATITUDE(nn.Module):
    """
    Compute root mean squared error (RMSE)
    """
    ...
    def __init__(self,
        ...
        super(RMSE_LATITUDE, self).__init__()
    ...
    def forward(self, predictions, targets):
        ...
        # Calculate the mean squared error
        # polar_squared_diff=torch.cat([squared_diff[:,0:16],squared_diff[:,105:121]],dim=-2)
        # Temperate_squared_diff=torch.cat([squared_diff[:,16:45],squared_diff[:,76:105]],dim=-2)
        # Tropical_squared_diff=squared_diff[:,45:76]
        # polar_squared_diff=squared_diff[:,101:121]
        # Temperate_squared_diff=squared_diff[:,81:101]
        # Tropical_squared_diff=squared_diff[:,61:81]
        polar_squared_diff=torch.cat([squared_diff[:,0:20],squared_diff[:,101:121]],dim=-2)
        Temperate_squared_diff=torch.cat([squared_diff[:,20:40],squared_diff[:,81:101]],dim=-2)
        Tropical_squared_diff=squared_diff[:,40:81]
        # polar_squared_diff=squared_diff[:,105:121]
        # Temperate_squared_diff=squared_diff[:,76:105]
        # Tropical_squared_diff=squared_diff[:,61:76]
        polar_mean_squared_error = torch.nanmean(polar_squared_diff)
        Temperate_mean_squared_error = torch.nanmean(Temperate_squared_diff)
        Tropical_mean_squared_error = torch.nanmean(Tropical_squared_diff)
        
        # Take the square root to get the RMSE
        polar_rmse,Temperate_rmse,Tropical_rmse = torch.sqrt(polar_mean_squared_error),torch.sqrt(Temperate_mean_squared_error),torch.sqrt(Tropical_mean_squared_error)
        
        return polar_rmse,Temperate_rmse,Tropical_rmse 

class RMSE_REGIONAL(nn.Module):
    """
    Compute root mean squared error (RMSE)
    """
    ...
    def __init__(self,
        ...
        super(RMSE_REGIONAL, self).__init__()
    ...
    def forward(self, predictions, targets):
        ...
        if self.lat_adjusted:
            squared_diff = self.weights.size(1) * (self.weights / torch.sum(self.weights)) * squared_diff 
        north_america_mask_grid=np.load('/home/zinanzheng/project/iclrs2s/S2S/draw/north_america_mask_grid.npy',allow_pickle=True)
        south_america_mask_grid=np.load('/home/zinanzheng/project/iclrs2s/S2S/draw/south_america_mask_grid.npy',allow_pickle=True)
        europe_mask_grid=np.load('/home/zinanzheng/project/iclrs2s/S2S/draw/europe_mask_grid.npy',allow_pickle=True)
        north_america_mask_grid = north_america_mask_grid ==0
        south_america_mask_grid = south_america_mask_grid==0
        europe_mask_grid = europe_mask_grid ==0

        north_america_squared_diff=squared_diff.clone()
        south_america_squared_diff=squared_diff.clone()
        europe_squared_diff=squared_diff.clone()

        north_america_squared_diff[:,north_america_mask_grid] = float('nan')
        south_america_squared_diff[:,south_america_mask_grid] = float('nan')
        europe_squared_diff[:,europe_mask_grid] = float('nan')
        polar_mean_squared_error = torch.nanmean(north_america_squared_diff)
        Temperate_mean_squared_error = torch.nanmean(south_america_squared_diff)
        Tropical_mean_squared_error = torch.nanmean(europe_squared_diff)
...
class ACC(nn.Module):
    ...
    def __init__(self,
        ...
        self.normalization_file = {
                'pressure': Path(config.DATA_DIR) / 'climatology_1.5' / 'climatology_pressure_level_1.5_new.zarr', 
                'single': Path(config.DATA_DIR) / 'climatology_1.5' / 'climatology_single_level_1.5_new.zarr', 
        }
        
        self.normalization_mean = {
                'pressure': xr.open_dataset(self.normalization_file['pressure'], engine='zarr')['mean'],
                'single': xr.open_dataset(self.normalization_file['single'], engine='zarr')['mean'],
        }
