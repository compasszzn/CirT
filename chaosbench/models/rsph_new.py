...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=[1, 242], stride=1, bias=bias)
...
class SphFormer(nn.Module):
    def __init__(
        ...
        self.token_embeds = PatchEmbed([121,242], input_size, embed_dim)
    ...
    def forward_encoder(self, x: torch.Tensor):
        ...
        x = torch.cat([real, -img], dim=-1)#torch.Size([16, 121, 256])
        x = torch.fft.irfft(x, self.dim, norm="forward")#torch.Size([16, 121, 256])
