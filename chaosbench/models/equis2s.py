...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.dim = dim
        self.attn_bias = nn.Parameter(torch.zeros(240, 240, 2), requires_grad=True)
...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=[img_size[0], 1], stride=1, bias=bias)
...
class EquiS2S(nn.Module):
    def __init__(
        ...
        # self.invsht = th.InverseRealSHT(121, 240, grid="equiangular")
        ...
        self.head.append(nn.Linear(embed_dim, self.input_size * 2 * self.patch_size))
    ...
    def unpatchify(self, x: torch.Tensor, h=None, w=None):
        ...
        imgs = x.reshape(shape=(x.shape[0], 2, c, h * p, w))
        return imgs
