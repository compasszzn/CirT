...
class SphFormer(nn.Module):
    def __init__(
        ...
        self.token_embeds_pre = PatchEmbed(img_size, 60, embed_dim)
        self.token_embeds_sin = PatchEmbed(img_size, 16, embed_dim)
        ...
        self.pos_bias = nn.Parameter(torch.zeros(1, img_size[0], img_size[1]), requires_grad=True)
        ...
        self.head.append(nn.Linear(embed_dim, self.input_size * self.pred_len * self.img_size[1]))
    ...
    def unpatchify(self, x: torch.Tensor, h=None, w=None):
        ...
        x = x.reshape(shape=(b, h, 1, p, c * self.pred_len))
        x = torch.einsum("nhwpc->nhcwp", x)
        x = x.reshape(shape=(b, 1, c * self.pred_len, h, p))
        # x = torch.view_as_complex(x)
        # x = self.invsht(x)
        # x = torch.fft.irfft(x, 240)
        imgs = x.reshape(shape=(b, self.pred_len, c, h, p))
        return imgs
