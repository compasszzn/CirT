...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=[244, 1], stride=1, bias=bias)
...
class SphFormer(nn.Module):
    def __init__(
        ...
        self.token_embeds = PatchEmbed([244,120], input_size, embed_dim)
    ...
    def forward_encoder(self, x: torch.Tensor):
        ...
        x = torch.fft.rfft(x, norm="forward", dim=-2)
        x = torch.view_as_real(x)
        ...
        x = torch.cat([real, -img], dim=-1)
        x = torch.fft.irfft(x, self.dim, norm="forward", dim=-1)
