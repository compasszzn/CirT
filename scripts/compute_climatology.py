...
def main(args):
    ...
    data_dir = Path(config.DATA_DIR) / 'era5_pressure_2005_2008'
    ...
    if args.is_spatial:
        ...
        elif args.dataset_name == 'single_levl':
            param_list = param_string = config.ERA5_SINGLE_LIST
        ...
        output_file = output_dir / f'climatology_{args.dataset_name}_old.zarr'
        ...
        for dataset_file in tqdm(dataset_files):
            ...
            elif args.dataset_name == 'pressure_level':
                ds = ds[config.ERA5_PRESSURE_LIST]
                
                # Only the first timestep is needed for instantenous forecast
                if args.dataset_name in list(config.S2S_CENTERS.keys()):
                    ds = ds.isel(step=0)
            elif args.dataset_name== 'single_level_merge':
                
                PARAMS = config.ERA5_SINGLE_LIST
                ds = ds[PARAMS]
            else:
            ...
            ds = ds.to_array().values#(6, 10, 361, 720)shape
        ...
        if (args.dataset_name == 'oras5') or (args.dataset_name == 'lra5'):
            ...
            param_string = [f"{param}-{level}" for param in config.ERA5_PRESSURE_LIST for level in config.PRESSURE_LEVELS]
        else: 
            param_string = [f"{param}-{level}" for param in config.ERA5_PARAMS for level in config.PRESSURE_LEVELS] 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name',default='pressure_level', help='single_level_merge,pressure_level')
    parser.add_argument('--is_spatial',default=0, type=int, help='If we perform climatology computation over the spatial grid...')
